...
export function getMediaType(rawUrl, opts = {}) {
  ...
  if (typeof rawUrl === "object") {
    if (rawUrl.type) return rawUrl.type;
    rawUrl = rawUrl.url || rawUrl.path || rawUrl.filename || "";
  }
 ...
 *
 * ✅ Cloudinary-first
 * ❌ Blocks local /uploads in production
 * ✅ Allows /uploads ONLY in local dev
 * ✅ Never guesses folders
 */
export function normalizeMediaUrl(rawUrl, opts = {}) {
  if (!rawUrl) return null;
  ...
  // Handle object form: { url, path, filename }
  if (typeof rawUrl === "object") {
    urlCandidate = rawUrl.url || rawUrl.path || rawUrl.filename || null;
  }
  ...
  // ✅ Absolute URLs (Cloudinary, CDN, etc.)
  if (/^https?:\/\//i.test(urlCandidate)) {
    return urlCandidate;
  }
  ...
  return `${backendRoot}${
    urlCandidate.startsWith("/") ? urlCandidate : `/${urlCandidate}`
  }`;
}
